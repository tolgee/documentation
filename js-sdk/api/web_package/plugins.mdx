...
Same as [`InContextTools`](./tools#incontexttools), except it only apply the plugin in dev mode (based on `process.env.NODE_ENV`), so it will be omitted in production (and won't extend your bundle size).
