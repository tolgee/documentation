...
Yes, there is a more straightforward solution which is called [text observer](./text-observer), which uses plain text for encoding the key info.

:::info
Mentioned behavior is part of [`ObserverPlugin`](./api/web_package/plugins#observerplugin), which is included in [`DevTools`](./api/web_package/plugins#devtools).
:::
