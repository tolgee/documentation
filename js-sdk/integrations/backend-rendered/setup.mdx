...
| `format` | Output format. `JSON_TOLGEE` creates simple `{"key": "translation"}` files. Other options include `JSON_ICU`, `XLIFF`, etc. See [CLI formats](/tolgee-cli/pull-command#format). |
