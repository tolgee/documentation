...
String. Api key to tolgee platform. Can be a [Personal Access Token or a Project API Key](/platform/account_settings/api_keys_and_pat_tokens).

:::danger Make sure you don't leak your API key
Use `apiKey` only if you are loading it from environment or other secured source (supported in .js or .yml files) or your config is not public.

In most cases it's better to use one-time [login command](#logging-in) or set it via the `TOLGEE_API_KEY` environment variable.
:::
