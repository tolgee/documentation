...
- `--patterns <patterns...>` (short: `-pt`) - File glob patterns to include (hint: make sure to escape it in quotes, or your hell might attempt to unroll some tokens like *)
- `--no-strict-namespace` - Don't require the namespace to always be detectable (at extraction).
- `--default-namespace <namespace>` - Default namespace used in extraction if not specified otherwise.
- `--parser <parser>` - Override parser detection. (choices: "react", "vue", "svelte")
