...
As we add new features, we inevitably have multiple versions (branches) of our application: the production version and development branches. It’s essential to know which keys belong to which branch since production keys require careful handling, while development keys can be modified or removed as needed.
...
:::info
This part expects you to have [@tolgee/cli](/tolgee-cli) installed and integrated into your project.
:::
...
So our system can now distinguish feature branches with `draft: ...` tag, we know which keys are in `production` and after some time we'll see `deprecated` keys. This system should be quite robust, but not bulletproof. If the extraction tool fails to extract some key, it can be marked as `deprecated` even though it's not, so we advise you to always check manually if the key can be deleted.

Other than that happy translating! 🚀

### Bonus: Why Not Branching?

We’ve considered adding branching to Tolgee, where the platform mirrors your branches. However, this involves significant technical challenges and usability issues. Git itself, while powerful, is complex and constantly evolving—a testament to the inherent difficulty of version control.
