...
i18next is a library and format commonly used for handling translations in JavaScript-based projects. Translations are usually stored as part of JavaScript code or in `.json` files. Tolgee supports importing the `.json` representation of translation mapping, but not all i18next features are supported.
...
Only specifiers specified in the table below are supported. Tolgee doesn't support additional flags and modifiers and won't convert such placeholders.
