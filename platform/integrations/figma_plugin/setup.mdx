...
`Default namespace` is applied to keys you newly create. It does not touch keys that already exist, so to move those you either edit the namespace field on the row, or select several and run the bulk `Set namespace` operation. See [namespaces](/platform/translation_keys/namespaces) for what they are.
