...
## Select Keys for Batch Operations
...
> To select multiple keys, click the key checkbox while holding `shift`, which will select all keys between the current one and your last selected one
...
- **Machine translation** – When you pick this operation, you will be prompted to select languages to translate into.
  The [primary Machine translation providers](/platform/projects_and_organizations/machine-translation-settings) chosen for the languages will be used to translate the keys.
...
- **Delete keys** – Moves selected keys to the [trash](trash). Keys remain in trash for 7 days before being permanently deleted, giving you time to restore them if needed.
