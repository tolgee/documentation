...
This is useful for performing operations on a many keys, such as deleting all keys in a namespace or machine translating all keys in a project.
...
You can also **select all keys** by clicking the "Select all" checkbox next to the batch operation dropdown list. When using the "Select all" checkbox after performing a search or applying filters, it will only select all the keys that match the search query or filter conditions.

Next, you can select the batch operation you want to perform from the dropdown list.

![Select all keys](/img/docs/platform/batch_select_all_keys.png)
