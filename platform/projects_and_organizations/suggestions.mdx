---
id: suggestions
title: Translation Suggestions
sidebar_label: Translation Suggestions
image: /img/og-images/platform.png
---

import { ScreenshotWrapper } from '../shared/_ScreenshotWrapper';


Suggestions allow users to propose changes to strings without editing them directly. The actual string value is changed after a reviewer approves the suggestion.

Suggestions are also how [community translation](./community_translation.mdx) works. On a public project, anyone signed in can suggest without becoming a member.

### How to enable suggestions

Go to `Project settings` > `Advanced` > `Suggestions mode`.

<ScreenshotWrapper
  src="/img/docs/platform/suggestions/settings.webp"
  alt="Project settings > Advanced > Suggestions mode"
/>

The setting applies to every way of creating a suggestion, not only to the Tolgee web app. While suggestions mode is off, the [Create translation suggestion](/api/create-suggestion) API endpoint rejects new suggestions with the `suggestions_disabled` error (HTTP 400), and the [in-context dialog](/js-sdk/in-context#suggest-translations-in-context) doesn't offer suggesting. Suggestions created earlier are not lost. They can still be listed, accepted, declined, and deleted.

### How to Add Translation Suggestions

Once suggestions mode is enabled, users can edit a translation or add a suggestion.

<ScreenshotWrapper
  src="/img/docs/platform/suggestions/suggestion-menu.webp"
  alt="Adding suggestion via menu"
/>

### Review and Approve Suggestions

When a translator adds a suggestion, it is visible in the translations view, but not active (meaning if translations are exported, there is still the old translation).

<ScreenshotWrapper
  src="/img/docs/platform/suggestions/suggestion-created.webp"
  alt="Created suggestion"
/>

Once a reviewer comes along, they can accept the suggestion.

<ScreenshotWrapper
  src="/img/docs/platform/suggestions/suggestion-accept.webp"
  alt="Accept suggestion"
/>

This is the moment when it will replace the existing translation.

<ScreenshotWrapper
  src="/img/docs/platform/suggestions/suggestion-accepted.webp"
  alt="Suggestion is accepted"
/>

When there are multiple suggestions on a string, you choose how accepting one affects the rest.
`Accept only` promotes the suggestion to the live translation and leaves the other suggestions
open, so you can keep comparing them. `Accept and decline others`, in the suggestion's `⋮` menu,
promotes the suggestion and dismisses the competing ones in a single step.

<ScreenshotWrapper
  src="/img/docs/platform/community-translation/suggestions-accept.webp"
  alt="Multiple suggestions on a string with the Accept only button and the Accept and decline others menu action"
/>

Tolgee shows up to three suggestions on a string at once. If there are more, use `Show all` to see
the rest. All previously accepted and declined suggestions are also found under `Show all`.

<ScreenshotWrapper
  src="/img/docs/platform/suggestions/suggestions-inactive.webp"
  alt="Inactive suggestions"
/>


### Protecting reviewed translations

Tolgee offers a mode where translators can only edit non-reviewed translations directly. You can enable this in advanced project settings:

<ScreenshotWrapper
  src="/img/docs/platform/suggestions/translation-protection-settings.webp"
  alt="Translation protection settings"
/>

So now, when a translator opens a reviewed translation, they can only suggest.

<ScreenshotWrapper
  src="/img/docs/platform/suggestions/translation-protection-suggest-only.webp"
  alt="Translation protection - suggest only"
/>

#### Granular protection

If you want more direct control over edit permissions and suggestions, you can do it through
[Granular permissions](/platform/projects_and_organizations/members#advanced-granular-permissions).

Here you can, for example, only allow the user to `Suggest`, so they can't edit any translation directly.

<ScreenshotWrapper
  src="/img/docs/platform/suggestions/suggestions-granular-permissions.webp"
  alt="Suggestions granular permissions"
/>

> Accepting the suggestions is tied to the `Change state` permission (which is what all reviewers have).

Deleting your own suggestion is a separate permission in the `Suggestions` group. Every [role](./members.mdx#permissions) from `View` up includes it. With granular permissions you can withhold it, and then the user can't take back a suggestion once it's sent. The `Manage` permission in the same group covers deleting anyone's suggestions, including your own.

On a [public project](./community_translation.mdx) this permission can't be withheld. Every signed-in user gets it there as part of the community permissions, including members whose granular permission leaves it out.

API keys and applications connected to your account, such as the Tolgee Tools browser plugin, carry their own permissions. Whatever the project is, they can delete the suggestions you made only when this permission is part of the key or of the access you granted.


