...
If a [character limit](/platform/translation_keys/keys#character-limit) is set on a key, the translation editor displays a character counter next to the translation input (e.g., `15/20`). When the translation exceeds the limit, the counter turns red and a warning message is shown.

Saving a translation that exceeds the character limit is still allowed, but a confirmation dialog will appear asking you to confirm. This lets translators work on longer drafts without being blocked, while still being aware of the limit.

<ScreenshotWrapper
  alt="Character limit exceeded in translation editor"
  src="/img/docs/platform/editing_translations/char_limit_exceeded.webp"
/>
