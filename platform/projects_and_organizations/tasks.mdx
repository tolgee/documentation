...
They can also see a notification next to his profile icon, where he can go to see his tasks.
...
Once the user clicks on `My tasks`, they can see a list of all tasks assigned to him.
...
Now every time the user changes a translation, it will be marked as done automatically (`Save & done` button), they can also change the `done` state by task button on the right side of the translation (it changes its color when the translation is done).
