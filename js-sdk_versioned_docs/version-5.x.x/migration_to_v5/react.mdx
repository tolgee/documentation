...
We've removed v4 `useCurrentLanguage` and `useSetLanguage` hooks, which are now replaced by the more general hook `useTolgee` ([read more](/integrations/react/switching_language.mdx)).
