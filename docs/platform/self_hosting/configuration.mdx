...
### tolgee.app-name
Name of the app, shown on the user interface. (default: `Tolgee`)
...
### tolgee.open-replay-api-key
API key for [OpenReplay](https://www.openreplay.com/). If unset, OpenReplay is disabled. (default: `null`)
...
Whether regular users are allowed to create organizations. When disabled, only administrators can create organizations
(default: `true`)
...
### use-redis
Whether Tolgee should use Redis to store clients. Useful if you have multiple instances of Tolgee for load balancing.
(default: `false`)

:::info
In a distributed environment, you **should** use a Redis server to ensure consistent broadcast of events to all
clients. For a simple single-node deployment, this is unnecessary.
:::
...
## Sendinblue
Prefix: `tolgee.send-in-blue-properties`

Configuration for the [Sendinblue](https://www.sendinblue.com/) marketting tool. Used by Tolgee Cloud.

### api-key
Sendinblue API key. (default: `null`)

### list-id
The marketting list ID users will be added/removed from. (default: `null`)
...
:::danger
While this is useful for unit tests, make sure to keep this **disabled** if you're importing projects on a production
