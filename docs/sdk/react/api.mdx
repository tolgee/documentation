...
Provides Tolgee context and takes care of running/stopping tolgee. Accepts [`Tolgee`](../api/core_package/tolgee) instance.
...
Use this hook to get `t` function, which can be used for imperative translating. You can also pass namespace(s), which you want to use. If no namespace is specified [`defaultNs`](../api/core_package/options#defaultns)
...
Read more about [`t` function](../api/core_package/tolgee#t)
...
Returns tolgee instance. Allows subscription to different [`events`](../api/core_package/events), which will trigger re-render.
