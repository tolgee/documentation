...
- parameter `options` - Tolgee configuration properties described in [`options`](/docs/sdk/api/core_package/options) section.
...
```ts
const translated = this.translateService.instantSafe(
  'this_is_a_key_with_params',
  { key: 'value' },
  'Default value'
);
