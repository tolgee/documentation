...
This plugin adds [`DevTools`](/docs/sdk/api/web_package/bundled-plugins#devtools) plugin automatically and also makes tolgee globally awailable, so it can be used even without `TolgeeProvider`.

```ts
tolgee.use(TolgeeProvider(options?: ReactOptions))
```
