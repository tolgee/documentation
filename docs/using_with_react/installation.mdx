---
id: installation
title: Installation
sidebar_label: Installation
slug: /using_with_react/installation
---

To get full image of working React integration check our [React example application](https://github.com/tolgee/react-sampleapp).
To install Tolgee React integration run:

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

<Tabs
    defaultValue="npm"
    values={[
        {label: 'npm', value: 'npm',},
        {label: 'yarn', value: 'yarn'},
    ]
    }>
<TabItem value="npm">

```shell
npm install @tolgee/react --save
```

</TabItem>
<TabItem value="yarn">

```shell
yarn add @tolgee/react
```

</TabItem>
</Tabs>

To install UI bundle for in-context translating you will also need to install package `@tolgee/ui`.

<Tabs
    defaultValue="npm"
    values={[
        {label: 'npm', value: 'npm'},
        {label: 'yarn', value: 'yarn'},
    ]
    }>
<TabItem value="npm">

```shell
npm install @tolgee/ui --save
```

</TabItem>
<TabItem value="yarn">

```shell
yarn add @tolgee/ui
```

</TabItem>
</Tabs>

First of all, you will need to wrap your application with `TolgeeProvider` component.

```javascript
import {TolgeeProvider} from "@tolgee/react";
import {UI} from "@tolgee/ui";

...

<TolgeeProvider
    filesUrlPrefix="i18n/"
    apiUrl={process.env.REACT_APP_TOLGEE_API_URL}
    apiKey={process.env.REACT_APP_TOLGEE_API_KEY}
    ui={process.env.REACT_APP_TOLGEE_API_KEY === "true" && UI}
>
    <Your app components>
</TolgeeProvider>
```

If you bootstrapped your application with Create React App, your .env file should look like this:

```shell
REACT_APP_TOLGEE_API_URL=https://app.tolgee.io
REACT_APP_TOLGEE_API_KEY=bkcq7tkoinjg9ehk9hc4t3duq3
REACT_APP_TOLGEE_WITH_UI=true
```

Otherwise, you can set the properties directly or you can use plugins like [dotenv-webpack plugin](https://www.npmjs.com/package/dotenv-webpack).