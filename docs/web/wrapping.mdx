...
How it works? Let's take basic example:
...
myElement.textContent = tolgee.t('test');
tolgee.on('keyUpdate', () => {
  myElement.textContent = tolgee.t('test');
});
...
Let's try to discover the secret by console.logging individual characters of [`t`](./core-package/tolgee#t) function result:
...
What are these empty characters at the end? They are not actually empty, only invisible
...
of translations. This method is very reliable and brings basically no overhead on developer side.
