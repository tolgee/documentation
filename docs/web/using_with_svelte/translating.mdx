...
slug: /web/using_with_svelte/translating
---
When you have Tolgee libraries installed and [`TolgeeProvider`](/docs/web/using_with_svelte/api#tolgeeprovider) is set
...
You can also provide default value as default slot of the [`T`](/docs/web/using_with_svelte/api#t-component) component. This value will be used when no translation
has been found in localization data.
```svelte
<T keyName="this_is_a_key">This is default.</T>
```

## `getTranslate` method
If you cannot use `T` component for some reason, then you can also use
[`getTranslate`](/docs/web/using_with_svelte/api#gettranslate-function) method returning store containing
