---
id: installation
title: Installation
sidebar_label: Installation
slug: /web/using_with_npm/installation
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

To install Tolgee run:
<Tabs
    defaultValue="npm"
    values={[
        {label: 'npm', value: 'npm',},
        {label: 'yarn', value: 'yarn'},
    ]
    }>
<TabItem value="npm">

```shell
npm install @tolgee/core --save
```

</TabItem>
<TabItem value="yarn">

```shell
yarn add @tolgee/core
```

</TabItem>
</Tabs>

To install UI bundle for in-context translating you will also need to install package `@tolgee/ui`.

<Tabs
    defaultValue="npm"
    values={[
        {label: 'npm', value: 'npm'},
        {label: 'yarn', value: 'yarn'},
    ]
    }>
<TabItem value="npm">

```shell
npm install @tolgee/ui --save
```

</TabItem>
<TabItem value="yarn">

```shell
yarn add @tolgee/ui
```

</TabItem>
</Tabs>

After installing packages you can use it like this:

```javascript
import {Tolgee} from "../clients/js/packages/core/lib/index";
import {UI} from "@tolgee/ui";

const tg = new Tolgee({
    apiKey: "your_api_key",
    apiUrl: "https://app.tolgee.io",
    ui: UI
})

pg.run();
```

Then you can use Tolgee instance the same way as described in [translating programmatically](../get_started/translating_programmatically).