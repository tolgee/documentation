...
For local development, supply `apiUrl`, `apiKey` options (see [Integration](/docs/platform/integration)) and [`DevTools`](./web-package/bundled-plugins#devtools) plugin. If you set everything properly, you can use in-context translating:
...
> NOTE: If you use Tolgee without any integration, make sure you call [`run`](./api/core-package/tolgee#run) method.
