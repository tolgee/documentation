...
Tools to manipulate different plugin interfaces.
...
Plugin which enables in-context translating. Check [Observer props](./observer-options).
...
`Backend` is used when `Tolgee` needs to fetch record (identified by language and namespace).
...
Method which allows plugins to override user set credentials to tolgee platform.
