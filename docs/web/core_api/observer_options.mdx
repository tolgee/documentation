...
slug: /web/core-api/observer-options
---

Tolgee Observer options, which can be passed to `tolgee.setObserverOptions` method.
...
### `options.inputPrefix` and `inputSuffix`
...
translated value. This mechanism is called [wrapping](/docs/web/wrapping).
