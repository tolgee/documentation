...
These strings are unique enough to not clash witch any other strings in your DOM, so it will not break your document.
...
If you are not using any JS framework, and you wish Tolgee to replace wrapped endoded texts even in production mode, set
this to `true`.
...
specified in this configuration properties.
...
an option of `select` HTML element cannot be used for capturing click event with `ALT` down. For these reasons, you can
