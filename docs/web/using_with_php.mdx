...
browser by tolgee JS library. See [hello world example](get_started/hello_world).

You can also provide translation parameters as second parameter of `translate` function.
...
[exporting them using Tolgee Web Application](http://localhost:3000/docs/server_and_web_app/exporting_translations).
...
### apiKey
Your secret key, which you can [obtain in web application](../server_and_web_app/obtaining_api_key). (string, default: empty string)

### defaultLanguage
The default language. (string, default: `en`)

### fallbackLanguage
The language used when current text is not available in currentLanguage. (string, default: `en`)
